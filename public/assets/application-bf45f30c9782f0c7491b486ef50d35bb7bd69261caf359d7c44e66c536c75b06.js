...
d;return b};this.dropNode=function(a){var d={};((a instanceof Array?a:[a])||[]).forEach(function(a){b.nodesIndex[a]?d[a]=!0:sigma.log('Node "'+a+'" does not exist.')});var c=[];b.nodes.forEach(function(a,b){a.id in d&&(c.unshift(b),0==a.degree&&delete d[a.id])});c.forEach(function(a){delete b.nodesIndex[b.nodes.splice(a,1)[0]['id']]});b.edges=b.edges.filter(function(a){return a.source.id in d?(delete b.edgesIndex[a.id],a.target.degree--,a.target.inDegree--,!1):a.target.id in d?(delete b.edgesIndex[a.id],a.source.degree--,a.source.outDegree--,
...
(function() {
  $(document).ready(function() {
    return $('#make-input-batch').bind('click', (function(_this) {
      return function(e) {
        return $('#resource_username').replaceWith(jQuery('<textarea />').attr({
          'name': 'resource[usernames]',
...
(function() {
  $(document).ready(function() {
    ...
    return $('.network-graph').each(function() {
      var theGraph;
      theGraph = this;
      return $(theGraph).find('.network-graph-canvas').each(function(i, e) {
        var hideSmallEdges, isRunning, sigInst, theContainer;
        theContainer = this;
        ...
        sigInst.parseJson($(e).attr('data-graph-url'), function() {
          return sigInst.draw();
        });
        hideSmallEdges = function(x) {
          var visible;
          visible = 0;
          sigInst.iterEdges((function(_this) {
            return function(e) {
              var weight;
              ...
              if (weight < x) {
                ...
                return sigInst._core.graph.nodesIndex[e.target].forceLabel = true;
              }
            };
          })(this));
          sigInst.draw();
        ...
        $(theGraph).find('.network-graph-filter').change(function() {
          var edgesCount, weight;
          ...
          if ($(this).is(':checked')) {
            ...
            sigInst.iterNodes((function(_this) {
              return function(n) {
                return n.forceLabel = false;
              };
            })(this));
            sigInst.draw();
          }
        ...
        (function() {
          var keepPopUp, onTarget, popUp;
          popUp = null;
          keepPopUp = false;
          onTarget = false;
          return sigInst.bind('overnodes', function(event) {
            var detailList, neighborEdges, node, nodes;
            nodes = event.content;
            neighborEdges = [];
            onTarget = true;
            sigInst.iterEdges(function(e) {
              if (nodes.indexOf(e.source) >= 0 || nodes.indexOf(e.target) >= 0) {
                return neighborEdges.push({
                  to: nodes.indexOf(e.source) >= 0 ? e.target : e.source,
                  size: e.size
                });
              } else {
                return e.hidden = 1;
            ...
            detailList = $('<ul>').append($('<li>').append($('<a>').attr('href', '/resource/' + node.id).text('Username: ' + node.id))).append($('<li>').text('Node size: ' + node.size)).append($('<li>').text('Coord: ' + node.x + "," + node.y));
            neighborEdges.sort(function(a, b) {
              return b.size - a.size;
            });
            $.each(neighborEdges, function(index, value) {
              return detailList.append($('<li>').text('Edge to: ' + value.to + ', weight: ' + value.size));
            });
            popUp = $('<div class="node-info-popup"></div>').append($('<button type="button" class="close">&times;</button>').click(function() {
              popUp && popUp.remove();
              popUp = false;
              return keepPopUp = false;
            })).append(detailList).attr('id', 'node-info' + sigInst.getID()).css({
              'left': node.displayX,
              'top': node.displayY + 15
            });
            return $(theContainer).append(popUp);
            ...
            return keepPopUp = true;
          }).bind('downgraph', function(e) {
            if (!onTarget) {
              keepPopUp = false;
              popUp && popUp.remove();
              return popUp = false;
        ...
        isRunning = false;
        $(theGraph).find('.toggle-layout').click(function() {
          if (isRunning) {
            isRunning = false;
            sigInst.stopForceAtlas2();
            return $(this).text('Start ForceAtlast Layout');
          } else {
            isRunning = true;
            sigInst.startForceAtlas2();
            return $(this).text('Stop ForceAtlast Layout');
          }
        });
        $(theGraph).find('.random-layout').click(function() {
          return sigInst.myRandomLayout();
        });
        $(theGraph).find('.circular-layout').click(function() {
          return sigInst.myCircularLayout();
        });
        return $(theGraph).find('.spiral-layout').click(function() {
          return sigInst.starWeightedLayout();
        });
      });
    });
  });
